...
## 🆕 2026-06-06 追加：/models 大規模 UX 改良（Airbnb 風 + シェア可能 URL）

`/models` ページを参考サイト（Airbnb / Booking / TripAdvisor 等）の慣習を踏まえ全面刷新。

### 追加コンポーネント
- `src/app/models/PlanCard.tsx` — グリッド/リスト両対応の単一カード
  - グリッド: 4:3 大画像 + ハート + 「人気」バッジ + 「N日間」バッジ
- `src/app/models/PlanList.tsx` — ソートメニュー + ビュー切替
- `src/app/models/QuickThemes.tsx` — ヒーロー直下の 11 テーマ + お気に入り ショートカット
- `src/app/models/PopularDestinations.tsx` — 人気エリアの横スクロールサムネ（タブ別 TOP8）
- `src/app/models/useFavorites.ts` — localStorage お気に入り hook

### 主要 UX 改善
- **ビュー切替**: グリッド ↔ リスト（localStorage で記憶、URL でも上書き可能）
- **お気に入り**: localStorage、絞り込み「お気に入りのみ」、件数バッジ
- **クイックテーマ**: 1-tap でテーマ切替
- **ソート**: おすすめ / 日数昇降 / タイトル
- **相互ハイライト**: カード hover → 地図ピン pulse / ピン hover → カード強調
- **ピンクリックでスムーズズーム**: 多数エリアクラスタは easeOutCubic で自動ズーム
- **URL クエリ同期**: tab/theme/sort/view/q/dest を URL に書き出し → シェア可能
- **シェアボタン**: ヒーロー右上、現在条件の URL をコピー（トースト通知）
- **キーボードショートカット**: `/` 検索、`g`/`l` ビュー切替、`Esc` 解除
- **モバイル**: フィルタは下から出る drawer、ヒーロー/人気エリアカードを縮小
- **sticky な選択チップ + 検索バー**: スクロール時もすぐ操作可能
- **空状態の CTA**: 条件を一括リセット
- **スピナー**: 地図読み込み中はリッチな円形スピナー
- **スクロールトップ**: 600px 以下スクロール時に右下に出現

### 地図側の改善（前段）
- クラスタ閾値 70→45 で初期から県・地域単位の分解能
- 高さ国内 580 / 海外 480、projection scale を 1.35/0.38 に微調整
- 地図右側にフィルタサイドバー（260px、狭幅では下に積む）
- テーマ/日数/出発地フィルタを地図上の数字とエリア集合にも反映

### 海外モデルプランの拡充（前段）
- アフリカ大幅追加: エチオピア・ウガンダ/ルワンダ（ゴリラ）・ボツワナ・キリマンジャロ・チュニジア・セネガル・ガーナ
- 中東: オマーン / サウジアラビア / レバノン / カタール
- 中央アジア・コーカサス: ウズベキスタン / ジョージア / イラン
- 南米追加: アタカマ砂漠 / ボゴタ / ベリーズ
- 周遊ルート 6 件と Wikipedia ランドマーク画像 23 枚を追加
- 合計 581 プラン（300 ルート × 平均 1.9 variant）が `/models` で閲覧可能
...
- `NewPageGap` は `visible=true` の時点では青いダッシュ枠だけを表示し、`active`（実際にカーソルが乗っている）のときだけ「＋ ここに配置」ラベルを表示する。これによりページ上の DropIndicator と非アクティブな gap のラベルが視覚的に重複しない（= 実際に挿入される箇所だけが明確に強調される）。
